...
training_arguments = AdaptationArguments(output_dir="train_dir",
                                         ...
                                         warmup_steps=1000,
                                         max_steps=100000,
                                         gradient_accumulation_steps=2,  # TODO: set
...
skofin_data_path = "data/skofin/skofin_qa_format_dump.json"
...
categories_en = [question.split()[0] if not question.startswith("To")
                 else " ".join(question.split()[:2])
                 for question in squad_train["question"]]
...
squad_cs_dataset = json.load(open("data/czech_squad.json"))
